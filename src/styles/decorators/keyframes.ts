...
export type KeyframesDefinition = {
  ...
  // Allow string keys for flexibility (e.g., '0%, 100%' for shared keyframes)
...
/**
 * Converts a KeyframesDefinition object to a CSS @keyframes string
 * 
 * @param name - The animation name
 * @param keyframes - The keyframes definition object
 * @returns Complete @keyframes CSS rule string
 * 
 * @example
 * keyframesObjectToString('spin', {
 *   '0%': { transform: 'rotate(0deg)' },
 *   '100%': { transform: 'rotate(360deg)' },
 * })
 * // Returns: '@keyframes spin { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } }'
 */
function keyframesObjectToString(name: string, keyframes: KeyframesDefinition): string {
  const keyframeRules = Object.entries(keyframes)
    .filter(([_, styles]) => styles !== null && styles !== undefined)
    .map(([selector, styles]) => {
      const cssContent = cssObjectToString(styles as CSSProperties);
...
export function Keyframes(name: string) {
  return function <This extends object, Value extends KeyframesDefinition | undefined>(
    ...
    context.addInitializer(function (this: any) {
      // Create a unique symbol for this keyframes rule's index
      // Using @keyframes as the "selector" for the index map
      const keyframesSelector = `@keyframes ${name}`;
      const indexSymbol = Symbol(`css_keyframes_${name}_${getterName}`);

      // Create a reactive effect that updates the keyframes when signals change
      const cleanup = effect(() => {
        // Call the getter to get the keyframes object (this tracks signals)
        ...
        // Convert the object to CSS @keyframes string
        const cssText = keyframesObjectToString(name, keyframesObject as KeyframesDefinition);
